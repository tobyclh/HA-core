...
def build_schema(
    ...
    my_nut = random.choice(NUT_NAMES)
    # _LOGGER.debug("Selected nut name: %s", my_nut)
    # _LOGGER.debug("Building schema with user_input: %s", user_input)
    # _LOGGER.debug("CONF_STILL_IMAGE_URL: %s", CONF_STILL_IMAGE_URL)
    spec = {
        vol.Optional(
            CONF_DEVICE_ALIAS,
            description={"suggested_value": user_input.get(CONF_DEVICE_ALIAS, my_nut)},
        ): str,
        vol.Required(
            CONF_DEVICE_IP,
            description={"suggested_value": user_input.get(CONF_DEVICE_IP, "127.0.0.1")},
        ): str,
        vol.Optional(
            CONF_STILL_IMAGE_URL,
            description={"suggested_value": user_input.get(CONF_STILL_IMAGE_URL, "https://cdn.wikimg.net/en/hkwiki/images/thumb/1/16/Hornet_Idle.png/150px-Hornet_Idle.png")},
        ): str,
        vol.Optional(
            CONF_STILL_IMAGE_URL,
...
class GenericIPCamConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        _LOGGER.debug("[CONFIG_FLOW] Starting user step with user_input: %s", user_input)
        errors = {}
        hass = self.hass
        if user_input:
...
class CameraImagePreview(HomeAssistantView):
    ...
    url = "/api/zukun/preview_flow_image/{flow_id}"
    name = "api:zukun:preview_flow_image"
    requires_auth = False
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "zukun_camera/start_preview",
...
async def ws_start_preview(
    ...
    if user_input.get(CONF_STILL_IMAGE_URL):
        ha_still_url = f"/api/zukun/preview_flow_image/{msg['flow_id']}?t={datetime.now().isoformat()}"
